...
from models.DIFF_transformer import RMSNorm, SwiGLU
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.double_conv = nn.Sequential(
            ...
            RMSNorm(d=out_channels, mode='image'),
            SwiGLU(d_model=out_channels, mode='image'),
...
class NodiffUNet(nn.Module):
    def __init__(self, n_channels=4, n_classes=1, bilinear=True):
        super(NodiffUNet, self).__init__()
...
if __name__ == "__main__":
    # 初始化模型
    model = NodiffUNet(n_channels=4, n_classes=1).cuda()
    # 测试前向传播
    input = torch.randn(1, 4, 96, 96).cuda()
    output = model(input)
    print(output.shape)
